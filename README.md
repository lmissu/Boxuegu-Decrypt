...
目前开发了Android APP，可以支持多个课程、子课程（课外拓展）、选择需要解密的课程，操作流程也很简单，考虑到性能问题，课程数量太多时不建议使用手机来解密，我都是用mumu模拟器下载课程安装解密软件操作的，将解密后的视频从共享文件夹复制到电脑即可。

java程序不支持多个课程解密，也不支持子课程解密，有实力的自行拉取源码编写实现相关功能。
